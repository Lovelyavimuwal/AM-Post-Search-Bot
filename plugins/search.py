...
async def search(bot, message):
    ...
    try:
        for channel in channels:
            async for msg in User.search_messages(chat_id=channel, query=query):
                if msg.text:
                    await send_message_in_chunks(bot, message.chat.id, msg.text)
                elif msg.photo:
                    await bot.send_photo(chat_id=message.chat.id, photo=msg.photo.file_id, caption=msg.caption or "")
                elif msg.video:
                    await bot.send_video(chat_id=message.chat.id, video=msg.video.file_id, caption=msg.caption or "")
                elif msg.document:
                    await bot.send_document(chat_id=message.chat.id, document=msg.document.file_id, caption=msg.caption or "")
                results_found = True
